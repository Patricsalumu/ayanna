<x-app-layout>
    <div class="max-w-lg mx-auto mt-10 bg-white rounded-xl shadow-lg p-6">
        ...
        <form method="POST" action="{{ route('comptes.update', $compte) }}">
                ...
                <select name="classe_comptable_id" class="w-full border rounded px-3 py-2" required onchange="updateTypeField(this)">
                    ...
                    @foreach($classesComptables as $classe)
                        <option value="{{ $classe->id }}" 
                                ...
                                @if(old('classe_comptable_id', $compte->classe_comptable_id) == $classe->id) selected @endif>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="numero" class="w-full border rounded px-3 py-2" required value="{{ old('numero', $compte->numero) }}" placeholder="Ex: 512000">
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="charge" @if(old('type', $compte->type)=='charge') selected @endif>Charge</option>
                    <option value="produit" @if(old('type', $compte->type)=='produit') selected @endif>Produit</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nom" class="w-full border rounded px-3 py-2" required value="{{ old('nom', $compte->nom) }}" placeholder="Ex: Banque">
                @error('nom')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Description</label>
                <input type="text" name="description" class="w-full border rounded px-3 py-2" value="{{ old('description', $compte->description) }}" placeholder="Description optionnelle">
    ...
    function updateTypeField(selectElement) {
        ...
        // Ne pas changer automatiquement si le type est déjà défini (mode édition)
